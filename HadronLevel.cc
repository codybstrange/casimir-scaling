...
bool HadronLevel::next( Event& event) {
  ...
  do {
    ...
    if (doHadronize) {
      ...
      //Save list of string directions with the rapidities of the end points
      if(casimirScaling) {
        vector< pair< pair<double,double>, pair<int,int> > > rapSpans =
         rapiditySpans(event);
        colConfig.rapSpans = rapSpans;
      }
...
vector< pair< pair<double, double>, pair<int,int> > >
  HadronLevel::rapiditySpans(Event& event){
  vector< pair< pair<double,double>, pair<int,int> > > rapSpans;
  for (int iSub = 0; iSub < int( colConfig.size() ); iSub++ ){
    pair< pair<double, double>, pair<int,int> > rapSpanSub;
    vector<int> iPartons = colConfig[iSub].iParton;
    //Junctions
    if (colConfig[iSub].hasJunction){
      double ymin = 1e10;
      ...
      //Push back the results, plus two more argument
      pair<double,double> raps = make_pair(ymin,ymax);
      pair<int,int> extraInfo  = make_pair(0,0);
      rapSpanSub = make_pair( raps, extraInfo );

    } else{
      int subSize = int( iPartons.size() );
      int end  = subSize - (colConfig[iSub].isClosed ? 0 : 1);
      double ymin = 1e10;
      double ymax = -1e10;
      for (int iP = 0; iP < end; iP++){
        int i1 = iPartons[iP];
        int i2 = iPartons[(iP+1)%subSize];
        double y1 = yMax(event[i1], MTINY);
        double y2 = yMax(event[i2], MTINY);
        double maxPair = max(y1,y2);
        double minPair = min(y1,y2);
        if( maxPair > ymax) ymax = maxPair;
        if( minPair < ymin) ymin = minPair;
      }
      int iStart = iPartons[0];
      double yStart = yMax(event[iStart],MTINY);
      double yEnd   = yMax(event[end],MTINY);
      int colFlowDir = 2;//Initialize as 2
      int pIDStart = event[iStart].id();
      //yStart > yEnd := "positive" flow := 1
           if ((yStart > yEnd) && pIDStart > 0) colFlowDir = 1;
      else if ((yEnd > yStart) && pIDStart > 0) colFlowDir = 0;
      else if ((yStart > yEnd) && pIDStart < 0) colFlowDir = 0;
      else if ((yEnd > yStart) && pIDStart < 0) colFlowDir = 1;

      pair<double,double> raps = make_pair(ymin,ymax);
      pair<int,int> extraInfo  = make_pair(pIDStart, colFlowDir);
      rapSpanSub = make_pair( raps, extraInfo );
    }
    rapSpans.push_back(rapSpanSub);
  }
  return rapSpans;
}
...
//--------------------------------------------------------------------------
/*
HadronLevel::coarseStrings(Event& event, double eClu){

  for (int iSub = 0; iSub < int( colConfig.size() ); iSub++){
    vector<int> iPartons = colConfig[iSub].iParton;
    // Ignore junctions
    if (colConfig[iSub].hasJunction) return 1;
    // Skip procedure if only two partons
    if (int(colConfig[iSub].size()) < 3) return 1;
    int subSize = int(iPartons.size());
    for (int iP = 0; iP < (subSize - 2); iP++){
      int ia = event[iP%subSize].id();
      int ir = event[(iP+1)%subSize].id();
      int ib = event[(iP+2)%subSize].id();
      double er = event[(iP+1)%subSize].e();
      double ma = event[iP%subSize].m();
      double mb = event[(iP+2)%subSize].m();
      int iaClu = ia;
      int ibClu = ib;
      if (ir == 21 && er < eClu) {
