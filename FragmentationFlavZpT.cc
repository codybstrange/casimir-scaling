  ...
  if (thermalModel || mT2suppression) {
    ...
    for (int q1 = 1; q1 < 6; q1++) {
      ...
      for (int q2 = 1; q2 < 6; q2++) if (q1!=q2) {
        baryonOctWeight[q1][q1][q2][1] = 0.1667; // qq1 + rs channels, but
...
FlavContainer StringFlav::pickGauss(FlavContainer& flavOld) {
  ...
  if ( (flavOld.id < 0 && flavOld.id > -9) || flavOld.id > 1000 )
    flavNew.id = -flavNew.id;
  //cout<<"Traffic through pickGauss, not pickCasimir"<<endl;
  return flavNew;
...
FlavContainer StringFlav::pickCasimir(FlavContainer& flavOld,
  double kappaNew) {
  ...
  //Scaled parameters
  double probQQtoQNew        = pow(probQQtoQ,(1./kappaNew) );
  double probStoUDNew        = pow(probStoUD, (1./kappaNew) );
  double probSQtoQQNew       = pow(probSQtoQQ, (1./kappaNew) );
  double probQQ1toQQ0New     = pow(probQQ1toQQ0, (1./kappaNew) );
  double popcornSmesonNew    = popcornSmeson;//pow(popcornSmeson, (1./kappaNew) );
  double popcornSpairNew     = popcornSpair;//pow(popcornSpair, (1./kappaNew) );
  double popcornRateNew      = popcornRate;//pow(popcornRate, (1./kappaNew) ); //Maybe change this

  //Diquark production scaling
  double probStoUDrootNew    = sqrt(probStoUDNew);
  double probSQtoQQrootNew   = sqrt(probSQtoQQNew);
  double probQQ1toQQ0rootNew = sqrt(probQQ1toQQ0New);

  //Derived parameters
  double probQandQQNew       = 1. + probQQtoQNew;
  double probQandSNew        = 2. + probStoUDNew;
  double probQandSinQQNew    = 2. + probSQtoQQNew * probStoUDNew;
  double probQQ1corrNew      = 3. * probQQ1toQQ0New;
  double probQQ1corrInvNew   = 1. / probQQ1corrNew;
  double probQQ1normNew      = probQQ1corrNew / (1. + probQQ1corrNew);
  ...
  barCGMax[ss1] = baryonCGMax[2];
  double dMBNew[8];
  dMBNew[ud0] = 2. * baryonCGSum[0] + probStoUDNew * baryonCGSum[1];
  dMBNew[ud1] = 2. * baryonCGSum[4] + probStoUDNew * baryonCGSum[5];
  dMBNew[uu1] = baryonCGSum[2] + (1. + probStoUDNew) * baryonCGSum[3];
  dMBNew[us0] = (1. + probStoUDNew) * baryonCGSum[0] + baryonCGSum[1];
  dMBNew[su0] = dMBNew[us0];
  dMBNew[us1] = (1. + probStoUDNew) * baryonCGSum[4] + baryonCGSum[5];
  dMBNew[su1] = dMBNew[us1];
  dMBNew[ss1] = probStoUDNew * baryonCGSum[2] + 2. * baryonCGSum[3];
  for (int i = 1; i < 8; ++i) dMBNew[i] /= dMBNew[0];

  double qBBNew[8];
  qBBNew[ud1] = probQQ1toQQ0rootNew;
  qBBNew[uu1] = probQQ1toQQ0rootNew;
  qBBNew[us0] = probSQtoQQrootNew;
  qBBNew[su0] = probStoUDrootNew * probSQtoQQrootNew;
  qBBNew[us1] = probQQ1toQQ0rootNew * qBBNew[us0];
  qBBNew[su1] = probQQ1toQQ0rootNew * qBBNew[su0];
  qBBNew[ss1] = probStoUDrootNew * pow2(probSQtoQQrootNew)
                  * probQQ1toQQ0rootNew;

  double qBMNew[8];
  qBMNew[ud1] = 3. * qBBNew[ud1];
  qBMNew[uu1] = 6. * qBBNew[uu1];
  qBMNew[us0] = probStoUDNew * qBBNew[us0];
  qBMNew[su0] = qBBNew[su0];
  qBMNew[us1] = probStoUDNew * 3. * qBBNew[us1];
  qBMNew[su1] = 3. * qBBNew[su1];
  qBMNew[ss1] = probStoUDNew * 6. * qBBNew[ss1];

  for (int i = 1; i < 8; ++i) qBBNew[i] = qBBNew[i] * qBMNew[i];

  qBMNew[us0] *= popcornSmesonNew;
  qBMNew[us1] *= popcornSmesonNew;
  qBMNew[ss1] *= popcornSmesonNew;

  double uNorm = 1. + qBMNew[ud1] + qBMNew[uu1] + qBMNew[us0] + qBMNew[us1];
  double scbBMNew[3];
  scbBMNew[0] = (2. * (qBMNew[su0] + qBMNew[su1]) + qBMNew[ss1]) / uNorm;
  scbBMNew[1] = scbBMNew[0] * popcornSpairNew * qBMNew[su0] / qBMNew[us0];
  scbBMNew[2] = (1. + qBMNew[ud1]) * (2. + qBMNew[us0]) / uNorm;
  for (int i = 1; i < 8; ++i) dMBNew[i] *= qBMNew[i];
  for (int i = 1; i < 8; ++i) qBMNew[i] *= barCGMax[i] / barCGMax[0];
  for (int i = 1; i < 8; ++i) qBBNew[i] *= barCGMax[i] / barCGMax[0];

////////// TBD
  double qNorm = uNorm * popcornRateNew / 3.;
  double sNorm = scbBMNew[0] * popcornSpairNew;
  double popFracNew = qNorm * (1. + qBMNew[ud1] + qBMNew[uu1] + qBMNew[us0]
    + qBMNew[us1] + sNorm * (qBMNew[su0] + qBMNew[su1] + 0.5 * qBMNew[ss1]))
     / (1. +  qBBNew[ud1]+ qBBNew[uu1] + 2. * (qBBNew[us0] + qBBNew[us1])
      + 0.5 * qBBNew[ss1]);
//////////

  double popSNew[3];
  popSNew[0] = qNorm * qBMNew[ud1] / qBBNew[ud1];
  popSNew[1] = qNorm * 0.5 * (qBMNew[us1] / qBBNew[us1]
    + sNorm * qBMNew[su1] / qBBNew[su1]);
  popSNew[2] = qNorm * sNorm * qBMNew[ss1]/qBBNew[ss1];

  double dWTNew[3][7];
  // Case 0: q -> B B.
  dWTNew[0][0] = (2. * (qBBNew[su0] + qBBNew[su1]) + qBBNew[ss1])
    / (1. + qBBNew[ud1] + qBBNew[uu1] + qBBNew[us0] + qBBNew[us1]);
  dWTNew[0][1] = 2. * (qBBNew[us0] + qBBNew[us1]) / (1. + qBBNew[ud1] + qBBNew[uu1]);
  dWTNew[0][2] = qBBNew[ss1] / (qBBNew[su0] + qBBNew[su1]);
  dWTNew[0][3] = qBBNew[uu1] / (1. + qBBNew[ud1] + qBBNew[uu1]);
  dWTNew[0][4] = qBBNew[su1] / qBBNew[su0];
  dWTNew[0][5] = qBBNew[us1] / qBBNew[us0];
  dWTNew[0][6] = qBBNew[ud1];

  // Case 1: q -> B M B.
  dWTNew[1][0] = (2. * (qBMNew[su0] + qBMNew[su1]) + qBMNew[ss1])
    / (1. + qBMNew[ud1] + qBMNew[uu1] + qBMNew[us0] + qBMNew[us1]);
  dWTNew[1][1] = 2. * (qBMNew[us0] + qBMNew[us1]) / (1. + qBMNew[ud1] + qBMNew[uu1]);
  dWTNew[1][2] = qBMNew[ss1] / (qBMNew[su0] + qBMNew[su1]);
  dWTNew[1][3] = qBMNew[uu1] / (1. + qBMNew[ud1] + qBMNew[uu1]);
  dWTNew[1][4] = qBMNew[su1] / qBMNew[su0];
  dWTNew[1][5] = qBMNew[us1] / qBMNew[us0];
  dWTNew[1][6] = qBMNew[ud1];

  // Case 2: qq -> M B; diquark inside chain.
  dWTNew[2][0] = (2. * (dMBNew[su0] + dMBNew[su1]) + dMBNew[ss1])
    / (1. + dMBNew[ud1] + dMBNew[uu1] + dMBNew[us0] + dMBNew[us1]);
  dWTNew[2][1] = 2. * (dMBNew[us0] + dMBNew[us1]) / (1. + dMBNew[ud1] + dMBNew[uu1]);
  dWTNew[2][2] = dMBNew[ss1] / (dMBNew[su0] + dMBNew[su1]);
  dWTNew[2][3] = dMBNew[uu1] / (1. + dMBNew[ud1] + dMBNew[uu1]);
  dWTNew[2][4] = dMBNew[su1] / dMBNew[su0];
  dWTNew[2][5] = dMBNew[us1] / dMBNew[us0];
  dWTNew[2][6] = dMBNew[ud1];


  // For original diquark assign popcorn quark and whether popcorn meson.
  int idOld = abs(flavOld.id);
  if (flavOld.rank == 0 && idOld > 1000) assignPopQ(flavOld);
  //assignPopQCasimir(flavOld,kappaNew);
  ...
  // Choose whether to generate a new meson or a new baryon.
  if (!doOldBaryon && !doPopcornMeson &&
    (probQandQQNew * rndmPtr->flat() > 1.)) {
    doNewBaryon = true;
    if ((1. + popFrac) * rndmPtr->flat() > 1.) flavNew.nPop = 1;
  }
  ...
  //Rewriting pickLightQ for Casimir Scaling
  // Single quark for new meson or for baryon where diquark already exists.
  if (!doPopcornMeson && !doNewBaryon) {
    double rndmFlav = probQandSNew * rndmPtr->flat();
    if (rndmFlav < 1.) flavNew.id = 1;
    if (rndmFlav < 2.) flavNew.id = 2;
    else flavNew.id = 3;
    if ( (flavOld.id > 0 && flavOld.id < 9) || flavOld.id < -1000 )
      flavNew.id = -flavNew.id;
  ...
  // Flavour of popcorn quark (= q shared between B and Bbar).
  if (doNewBaryon) {
    double sPopWT = dWTNew[iCase][0];
    if (iCase == 1) sPopWT *= scbBMNew[0] * popcornSpairNew;
    double rndmFlav = (2. + sPopWT) * rndmPtr->flat();
    flavNew.idPop = 1;
    if (rndmFlav > 1.) flavNew.idPop = 2;
    if (rndmFlav > 2.) flavNew.idPop = 3;
  ...
  // Flavour of vertex quark.
  double sVtxWT = dWTNew[iCase][1];
  if (flavNew.idPop >= 3) sVtxWT = dWTNew[iCase][2];
  if (flavNew.idPop > 3) sVtxWT *= 0.5 * (1. + 1./dWTNew[iCase][4]);
  double rndmFlav = (2. + sVtxWT) * rndmPtr->flat();
  flavNew.idVtx = 1;
  if (rndmFlav > 1.) flavNew.idVtx = 2;
  if (rndmFlav > 2.) flavNew.idVtx = 3;
  ...
  if (flavNew.idPop < 3 && flavNew.idVtx < 3) {
    flavNew.idVtx = flavNew.idPop;
    if (rndmPtr->flat() > dWTNew[iCase][3]) flavNew.idVtx = 3 - flavNew.idPop;
  }

  // Pick 2 * spin + 1.
  int spin = 3;
  if (flavNew.idVtx != flavNew.idPop) {
    double spinWT = dWTNew[iCase][6];
    if (flavNew.idVtx == 3) spinWT = dWTNew[iCase][5];
    if (flavNew.idPop >= 3) spinWT = dWTNew[iCase][4];
    if ((1. + spinWT) * rndmPtr->flat() < 1.) spin = 1;
  }
  ...
  //cout<<"new rho(s)  "<<probStoUDNew
  //    <<"new popcornRate "<<popcornRateNew<<endl;
  return flavNew;
  /*
  // Case: 0 = q -> B B, 1 = q -> B M B, 2 = qq -> M B.
  int iCase = flavNew.nPop;
...
void StringFlav::assignPopQCasimir(FlavContainer& flav, double kappaNew){

  //Safety check
  int idAbs = abs(flav.id);
  if (flav.rank > 0 || idAbs < 1000) return;

  //Choose popcorn quarks
  int id1 = (idAbs/1000)%10;
  int id2 = (idAbs/100)%10;
  double pop2WT = 1.;

  double prob1  = sqrt(pow(probStoUD, (1./kappaNew) ) );
  double prob2  = sqrt(pow(probSQtoQQ, (1./kappaNew) ) );
  double prob3  = sqrt(pow(probQQ1toQQ0, (1./kappaNew) ) );
  double prob4  = pow(popcornSmeson, (1./kappaNew) );
  double popcornSpairNew = pow(popcornSpair, (1./kappaNew) );

  double qBMud1, qBMuu1, qBMus0, qBMus1, qBMss1, qBMsu1, qBMsu0;
  qBMud1 = 3. * prob3;
  qBMuu1 = 6. * prob3;
  qBMus0 = pow2(prob1) * prob2 * prob4;
  qBMsu0 = prob1 * prob2;
  qBMus1 = pow2(prob1) * 3. * prob3 * prob2 * prob4;
  qBMsu1 = 3. * prob3 * prob1 * prob2;
  qBMss1 = pow3(prob1) * 6. * pow2(prob2) * prob3 * prob4;

  double norm   = 1. + qBMud1 + qBMuu1 + qBMus0 + qBMus1;
  double scbBM0 = (2. * (qBMsu0 + qBMsu1) + qBMss1)/norm;
  double scbBM1 = scbBM0 * popcornSpairNew * qBMsu0 / qBMus0;
  double scbBM2 = (1. + qBMud1) * (2. + qBMus0) / norm;
  if      (id1 == 3) pop2WT = scbBM1;
  else if (id1 >  3) pop2WT = scbBM2;
  if      (id2 == 3) pop2WT /= scbBM1;
  else if (id2 >  3) pop2WT /= scbBM2;
  ...
  // Also determine if to produce popcorn meson.
  flav.nPop = 0;
  //Assume popcorn rate scales with casimir scaling
  double popRate = pow(popcornRate, (1./kappaNew) );
  double normNew = norm * popRate / 3.;
  double snorm   = scbBM0 * popcornSpairNew;
  double popS0, popS1, popS2;
  popS0 = normNew * qBMud1 / ( prob3 );
  popS1 = normNew * 0.5 * (qBMus1 / (prob3 * prob2)
    + snorm * qBMsu1 / (prob3 * prob2 * prob1) );
  popS2 = normNew * snorm * qBMss1 / (prob1 * pow2(prob2) * prob3);
  double popWT = popS0;
  if (id1 == 3) popWT = popS1;
  if (id2 == 3) popWT = popS2;
  if (idAbs%10 == 1) popWT *= prob3;
  if ((1. + popWT) * rndmPtr->flat() > 1.) flav.nPop = 1;
}
...
void StringPT::init(Settings& settings,  ParticleData* particleDataPtrIn,
  ...
  // Casimir scaling
  casimirScaling   = settings.flag("StringPT:casimirScaling");
  //casBaryScaling   = settings.flag("StringPT:casBaryScaling");
...
//CASIMIR SCALING PT
pair<double,double> StringPT::pxyCasimir(int idIn, double kappaNew){
  //Modify the width of the Gaussian
  double sigmaNow = sigmaQ;
  //sigmaNow *= sqrt(kappaNew);
  if(rndmPtr->flat() < enhancedFraction) sigmaNow *= enhancedWidth;

  if (useWidthPre) {
    if (abs(idIn) > 10) sigmaNow *= widthPreDiquark;
    sigmaNow *= pow(widthPreStrange, particleDataPtr->nQuarksInCode(idIn, 3) );
  }

  pair<double, double> gauss2 = rndmPtr->gauss2();
  return pair<double, double>(sigmaNow * gauss2.first, sigmaNow * gauss2.second);
...
double StringPT::BesselK0(double x){
  if( (x < 1.) && (x > 0.) ){
    double P7 =   1.1593151565841244842077226e-01           +
                  2.7898287891460317300886539e-01*x         +
                  2.5248929932161220559969776e-02*pow2(x)   +
                  8.4603509072136578707676406e-04*pow3(x)   +
                  1.4914719243067801775856150e-05*pow(x,4)  +
                  1.6271068931224552553548933e-07*pow(x,5)  +
                  1.2082660336282566759313543e-09*pow(x,6)  +
                  6.6117104672254184399933971e-12*pow(x,7)  ;

    double d = pow2(x/2);//Dummy variable for shorthand for P6
    double P6 = 1.0000000000000000044974165e+00             +
                2.4999999999999822316775454e-01*d           +
                2.7777777777892149148858521e-02*pow2(d)     +
                1.7361111083544590676709592e-03*pow3(d)     +
                6.9444476047072424198677755e-05*pow(d,4)    +
                1.9288265756466775034067979e-06*pow(d,5)    +
                3.9908220583262192851839992e-08*pow(d,6)    ;

    double I0 = d*P6 + 1.;
    double result = P7 - log(x)*I0;
    if(result > 0.) return result;
    else{cout<<"K0 < 0. for 0. < x < 1."<<endl; return 1;}
  }
  else if(x < 0)
  {
    cout<<"Argument is negative"<<endl;
    return 1;
  }
  else{
    double d = 1./x;//Dummy variable shorthand for P21 & Q2
    double Q2 = 8.5331186362410449871043129e-02             +
                7.3477344946182065340442326e-01*d           +
                1.4594189037511445958046540e+00*pow2(d)     ;

    double P21 = 1.0694678222191263215918328e-01            +
                  9.0753360415683846760792445e-01*d         +
                  1.7215172959695072045669045e+00*pow2(d)   +
                  -1.7172089076875257095489749e-01*pow3(d)  +
                  7.3154750356991229825958019e-02*pow(d,4)  +
                  -5.4975286232097852780866385e-02*pow(d,5) +
                  5.7217703802970844746230694e-02*pow(d,6)  +
                  -7.2884177844363453190380429e-02*pow(d,7) +
                  1.0443967655783544973080767e-01*pow(d,8)  +
                  -1.5741597553317349976818516e-01*pow(d,9) +
                  2.3582486699296814538802637e-01*pow(d,10) +
                  -3.3484166783257765115562496e-01*pow(d,11)+
                  4.3328524890855568555069622e-01*pow(d,12) +
                  -4.9470375304462431447923425e-01*pow(d,13)+
                  4.8474122247422388055091847e-01*pow(d,14) +
                  -3.9725799556374477699937953e-01*pow(d,15)+
                  2.6507653322930767914034592e-01*pow(d,16) +
                  -1.3951265948137254924254912e-01*pow(d,17)+
                  5.5500667358490463548729700e-02*pow(d,18) +
                  -1.5636955694760495736676521e-02*pow(d,19)+
                  2.7741514506299244078981715e-03*pow(d,20) +
                  -2.3261089001545715929104236e-04*pow(d,21);

    double result = sqrt(d)*P21/(Q2*exp(x));
    if(result > 0.) return result;
    else{cout<<"K0 < 0. for x > 1."<<endl; return 1;}
  }
}
