...
public:
  ...
  virtual FlavContainer pickC(FlavContainer& flavOld, double nNSP = 0.0) {
    hadronIDwin = 0; idNewWin = 0; hadronMassWin = -1.0;
    return pickCasimir(flavOld, nNSP);}
  virtual FlavContainer pickCasimir(FlavContainer& flavOld, double nNSP);
  ...
  //same as above, but applied to Casimir scaling
  /*virtual int getHadronIDC(FlavContainer& flav1, FlavContainer& flav2,
    double nNSC = 0., bool finalTwoCasimir = false) {
    if (finalTwoCasimir) return ((casimirScaling) ?
      combineLastCasimir(flav1, flav2, nNSC) : combine(flav1, flav2));
    if ((casimirScaling) && (hadronIDwin != 0)
      && (idNewWin != 0)) return getHadronIDwin();
    return combine(flav1, flav2); }*/
  ...
  void assignPopQCasimir(FlavContainer& flav, double kappaNew = 1.);
...
private:
  ...
  bool   closePacking, casimirScaling;
...
public:
  ...
  pair<double, double>  pxy(int idIn, double nNSP = 0.0) {
    if(thermalModel) return pxyThermal(idIn, nNSP);
    else if(casimirScaling) return pxyCasimir(idIn, nNSP);
    else return pxyGauss(idIn, nNSP); }
  pair<double, double>  pxyGauss(int idIn = 0, double nNSP = 0.0);
  pair<double, double>  pxyThermal(int idIn, double nNSP = 0.0);
  pair<double, double>  pxyCasimir(int idIn, double nNSP = 0.0);


  // Gaussian suppression of given pT2; used in MiniStringFragmentation.
  double suppressPT2(double pT2) {
    if(thermalModel) return exp(-sqrt(pT2)/temperature);
    else return exp(-pT2/sigma2Had); }
...
private:
  ...
  // Evaluate Bessel function K_0(x).
