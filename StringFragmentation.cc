...
void StringEnd::newHadron(double nNSP) {
  ...
  else if (casimirScaling) {

    do{
      flavNew = flavSelPtr->pickC( flavOld, nNSP);
      idHad   = flavSelPtr->combine( flavOld, flavNew);
    } while (idHad == 0);

    pair<double,double> pxy = pTSelPtr->pxyCasimir(flavNew.id, nNSP);
    pxNew = pxy.first;
    pyNew = pxy.second;
    pxHad = pxOld + pxNew;
    pyHad = pyOld + pxNew;
...
bool StringFragmentation::fragment( int iSub, ColConfig& colConfig,
  ...
  //Rapidity pairs [yMin,yMax] of colour singlet systems (Casimir scaling)
  vector< pair< pair<double,double>, pair<int,int> > > rapSpans =
    colConfig.rapSpans;
  ...
  for ( int iTry = 0; ; ++iTry) {
    ...
    for ( ; ; ) {
      ...
      //CASIMIR SCALING Check number of strings in either direction
      int stringDir = int( rapSpans[iSub].second.second );
      vector< pair< pair<double, double>, pair<int, int> > > otherrapSpans =
        rapSpans;
      otherrapSpans.erase(otherrapSpans.begin()+iSub);
      double kappaNew = (casimirScaling) ? nearStringsCasimir(nowEnd,
        otherrapSpans, stringDir) : 1.;
      ...
      if(casimirScaling){
        nowEnd.newHadron(kappaNew);
      } else nowEnd.newHadron(nNSP);
...
double StringFragmentation::nearStringPieces(StringEnd end,
  ...
  double kappaNew = 1. + nString*0.25;
  return kappaNew;
...
double StringFragmentation::nearStringsCasimir(StringEnd end,
  vector< pair< pair<double,double>, pair<int,int> > >& otherrapSpans,
  int stringDir){

  if (int(otherrapSpans.size()) < 1) return 1;
  //Ignore junctions
  if (hasJunction) return 1;
  //Get temporary hadron momentum
  double phi      = 2.*M_PI*rndmPtr->flat();
  double mult     = -1.;
  int nTryMax     = 100;
  double multStep = 5. / ((double)nTryMax/2.);
  double multNow  = 1. + multStep;
  Vec4 pHad       = Vec4(0.,0.,0.,-1.);
  ...
  //In case of failure, use beam remnant momentum
  if (pHad.e() < 0.) pHad = pRem;
  Particle hadron = Particle();
  hadron.p(pHad); hadron.m(pHad.mCalc());
  double yHad = hadron.y();
  int nPlus  = 0;
  int nMinus = 0;
  double kappaNew = 1.;
  for (int iSub = 0; iSub < int(otherrapSpans.size()); iSub++) {
    pair< pair<double,double>, pair<int,int> > pairNow = otherrapSpans[iSub];
    double y1 = double(pairNow.first.first);
    double y2 = double(pairNow.first.second);
    int pIDStart = int(pairNow.second.first);
    int colFlowDir = int(pairNow.second.second);
    //int hadColFlowDir = rndmPtr->flat();
    //if (end == posEnd) hadColFlowDir = 1;
    //else if( end == negEnd ) hadColFlowDir = 2;
    //Assign the hadron some random colour flow direction
    //If the number is less than 0.5, then it is flowing in "negative" dir
    if ( (y1 < yHad) && (yHad < y2) ){
      //Gluon loop
      if ( pIDStart == 21 ){ nMinus++; nPlus++;}
      if ( (abs(pIDStart) < 6) ){
        //Ordinary q\bar{q} strings
        if ( colFlowDir == stringDir ) nPlus++;
        else nMinus++;
      }
    }
  }
  /*if (nPlus != 0 || nMinus != 0){
    ...
    event.list();}*/
  kappaNew += 0.25*nPlus + 0.125*nMinus;
